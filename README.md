...
| `/dashboard`           | `DashboardComponent`          | Resumen por rol, apps y jobs recientes |
| `/profile`             | `ProfileComponent`            | Edición de perfil y contraseña         |
| `/admin/identity`      | `IdentityAdminComponent`      | Consola de usuarios, grupos y permisos |
...
- `adminGuard`: permite acceso a `/admin/identity` solo a roles `root` y `admin`.
- `appAccessGuard`: valida si el `route_key` de una app está habilitado para la sesión actual; si no, redirige al hub `/apps`.
...
La plataforma no es solo un catálogo de apps científicas: incorpora un dominio transversal de identidad con JWT, roles, grupos de trabajo y permisos por app.
