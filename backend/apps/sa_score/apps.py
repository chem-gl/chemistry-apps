"""apps.py: Configuración Django de la app SA Score.
...
Cómo se usa:
- Django instancia `SaScoreConfig` al cargar `INSTALLED_APPS`.
- En `ready()`, se valida unicidad de plugin/rutas y se importa `plugin.py`
  para activar el decorador `@PluginRegistry.register("sa-score")`.
"""
...
class SaScoreConfig(AppConfig):
    """Registra la app SA Score (accesibilidad sintética) en el ecosistema científico."""
