...
y orquestar el cálculo completo de constante de velocidad TST.
Usado por plugin.py en la etapa de compute del job.
"""
...
def _compute_thermodynamic_terms(
    *,
    ...
    count_reactants: int = sum(1 for s in [react_1, react_2] if s["is_provided"])
    count_products: int = sum(1 for s in [product_1, product_2] if s["is_provided"])
    ...
    gibbs_r1, gibbs_r2 = react_1["free_energy"], react_2["free_energy"]
    gibbs_ts = transition_state["free_energy"]
    gibbs_p1, gibbs_p2 = product_1["free_energy"], product_2["free_energy"]
    ...
    if cage_effects and delta_n_transition != 0:
        ...
        emit_log(
            ...
            {"cage_correction": cage_correction, "count_reactants": count_reactants},
        )
...
def _compute_diffusion_terms(
    *,
    ...
    viscosity_pa_s = _resolve_viscosity(
        solvent=solvent, custom_viscosity=custom_viscosity
    )
