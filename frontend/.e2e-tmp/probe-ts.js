...
(async () => {
  ...
  const api = [];
  p.on('response', r => { if (r.url().includes('/api/')) api.push(`${r.status()} ${r.request().method()} ${r.url().slice(-56)}`); });
  ...
  await p.setInputFiles('.easy-rate-shell .entry-panel .file-slot-card:nth-child(1) input[type=file]', '/tmp/opencode/agalano2-ts-fix.log');
  ...
  const preview = await p.evaluate(() => {
    ...
    return {
      ...
      preview: card ? (card.querySelector('.preview-grid') || {}).innerText?.slice(0, 400) : null,
    };
  });
